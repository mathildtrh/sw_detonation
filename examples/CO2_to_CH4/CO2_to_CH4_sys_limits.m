...
% txt="Computing FPR-TNR boundary (Yann), this may take a while";
% disp(txt)
...
txt="Computing FPR-TNR boundary (Mathilde), this may take a while";
disp(txt)
nchis_tnr_M=100; %number of points on chi axis
nys_tnr_M=30; %number of points in computed polars
chis_FPR_TNR_M=linspace(.34,1,nchis_tnr_M); %chi values
omegas_FPR_TNR_M=zeros(1,nchis_tnr_M);%pre-allocated omega array
...
hold on
plot(omegas_RRR_BPR,chis_RRR_BPR,'r') %ploting RRR<->BPR limit Yann's method
hold on
...
hold on
% plot(omegas_FPR_TNR,chis_FPR_TNR,'c') %ploting FPR<->TNR lim Yann
% hold on
plot(omegas_FPR_TNR_M,chis_FPR_TNR_M,'--c') %ploting FPR<->TNR lim Mathilde-Takayama
hold on
...
legends={"RRE<->RRR (analytical resolution)",...
    ...
    "RRR<->BPR (Yann)","RRR<->BPR (Mathilde)","BPR<->FNR","FPR<->TNR",...
    "TNR<->LSR",...

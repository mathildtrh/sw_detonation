...
press_jumps = linspace(1.02, 1.66, 65);
...
for mach=1:nM
    [sol]=RREevol(cst, X, Y, mach_numbers(mach), omega_deg, press, temp_I, temp_II, 0); 
    xi_t(mach) = sol(2,4)/sol(2,5);
    xi_r(mach) = sol(2,3)/sol(2,2);
end

%figure()
plot(mach_flow,xi_t, 'LineWidth', 2);
hold on
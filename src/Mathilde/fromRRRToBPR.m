function is_sf_RR=fromRRRToBPR(xi,omega_rad,gamma_I,gamma_II,mu_I,...
    mu_II,ny,varargin)
%determines whether refraction will be regular or not...
    ...
    Mi=sqrt(xiToSqMach(xi,gamma_I,omega_rad));
    %Mi=Msh/sin(omega_rad);
    [i_xis,i_deltas]=getPolarMathilde(Mi,gamma_I,ny);
    %computing transmitted polar
    Mt=sqrt(temp_ratio*gamma_I*mu_II/(gamma_II*mu_I))*Mi;
    [t_xis,t_deltas]=getPolarMathilde(Mt,gamma_II,ny);
    max_t_xi=max(t_xis);
    if max_t_xi<=xi
